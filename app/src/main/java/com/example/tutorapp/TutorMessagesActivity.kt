...
class TutorMessagesActivity : AppCompatActivity() {
    ...
    private var studentList: List<Users> = emptyList()
    private var selectedStudentID: String = "" // Selected student ID

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.tutor_activity_messages)
        ...
        studentSpinner = findViewById(R.id.student_spinner) // Update to student_spinner
        ...
        studentSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View, position: Int, id: Long) {
                selectedStudentID = studentList[position].id ?: "" // Get selected student ID
                loadMessages() // Load messages for the selected student
            }

            override fun onNothingSelected(parent: AdapterView<*>) {
                selectedStudentID = "" // Default or handle case when nothing is selected
            }
        }
        ...
        sendButton.setOnClickListener {
            ...
            if (messageText.isNotEmpty() && selectedStudentID.isNotEmpty()) {
                sendMessage(messageText, selectedStudentID)
                messageEditText.text.clear() // Clear input field
            } else {
                Toast.makeText(this, "Please select a student and enter a message", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun loadMessages() {
        ...
        Log.d("MessagesActivity", "Loading messages for student ID: $selectedStudentID")
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (snapshot in dataSnapshot.children) {
                        ...
                        if (message != null && message.senderID == selectedStudentID) {
        ...
        database.orderByChild("senderID").equalTo(currentUserID)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (snapshot in dataSnapshot.children) {
                        val message = snapshot.getValue(Message::class.java)
                        if (message != null && message.receiverID == selectedStudentID) {
    ...
    private fun loadStudents() {
        ...
        usersRef.orderByChild("role").equalTo("student").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                val newStudentList = mutableListOf<Users>()
                for (snapshot in dataSnapshot.children) {
                    val student = snapshot.getValue(Users::class.java)
                    student?.let {
                        val studentWithId = it.copy(id = snapshot.key)
                        newStudentList.add(studentWithId)
                    }
                }
                studentList = newStudentList

                // Populate Spinner
                val adapter = ArrayAdapter(this@TutorMessagesActivity, android.R.layout.simple_spinner_item, studentList.map { it.fullName })
                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                studentSpinner.adapter = adapter
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@TutorMessagesActivity, "Error loading students: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
